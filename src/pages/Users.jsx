...
export const Users = () => {
  ...
  const userSummary = useMemo(() => {
    return {
      ...
      pending: users.filter((user) => user.status === 'Pending Login').length,
      online: users.filter((user) => user.on_duty).length,
    };
  }, [users]);
  ...
  try {
    ...
    if (editingUser) {
      ...
      const result = await api.post("/create-worker", {
        name: formData.name,
        email: formData.email,
        phone: formData.phone,
        code: generatedCode,
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 bg-slate-50 min-h-screen max-w-[1600px] mx-auto relative">
      <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(163,230,53,0.28),_transparent_38%),linear-gradient(135deg,#0f172a_0%,#111827_45%,#1e293b_100%)] p-6 sm:p-8 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <p className="text-[11px] font-black uppercase tracking-[0.28em] text-primary/90 text-center md:text-left">Admin Directory</p>
            <h2 className="text-2xl sm:text-4xl font-black tracking-tight text-white text-center md:text-left">Worker Directory</h2>
            <p className="text-slate-300 font-medium text-sm sm:text-base text-center md:text-left max-w-2xl">Manage worker onboarding, status, and identity details from one cleaner control surface.</p>
          </div>
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-3.5 text-sm font-bold text-white shadow-xl shadow-primary/30 hover:brightness-95 transition-all transform active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { label: 'Total Workers', value: userSummary.total },
          { label: 'Verified', value: userSummary.verified },
          { label: 'Pending Login', value: userSummary.pending },
          { label: 'On Duty', value: userSummary.online },
        ].map((item) => (
          <div key={item.label} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
            ...
            <div className="mt-4 h-1.5 rounded-full bg-slate-100 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-primary via-lime-300 to-emerald-300" style={{ width: `${Math.max(18, Math.min(100, item.value ? (item.value / Math.max(userSummary.total, 1)) * 100 : 18))}%` }} />
            </div>
          </div>
        ))}
      </div>
