...
- Add skill points for the course
- Skill points for the course should be be maximum 500 points
...
export const AITools = () => {
  ...
  const courseSummary = {
    total: generatedCourses.length,
    published: generatedCourses.filter((course) => course.status === 'Published').length,
    withVideo: generatedCourses.filter((course) => course.hasVideo).length,
    advanced: generatedCourses.filter((course) => course.difficulty === 'Advanced').length,
  };
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8 bg-slate-50 min-h-screen animate-in fade-in duration-700">
      {/* Header Section */}
      <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(56,189,248,0.22),_transparent_32%),radial-gradient(circle_at_bottom_right,_rgba(163,230,53,0.26),_transparent_28%),linear-gradient(135deg,#082f49_0%,#0f172a_55%,#1e293b_100%)] p-6 sm:p-8 shadow-xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-2xl bg-white px-8 py-4 text-sm font-black text-slate-900 shadow-xl shadow-slate-950/20 hover:bg-slate-100 transition-all transform active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { label: 'Total Courses', value: courseSummary.total },
          { label: 'Published', value: courseSummary.published },
          { label: 'With Video', value: courseSummary.withVideo },
          { label: 'Advanced', value: courseSummary.advanced },
        ].map((item) => (
          <div key={item.label} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
            ...
            <div className="mt-4 h-1.5 rounded-full bg-slate-100 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-sky-400 via-primary to-lime-300" style={{ width: `${Math.max(20, Math.min(100, item.value ? (item.value / Math.max(courseSummary.total, 1)) * 100 : 20))}%` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {generatedCourses.map((course) => (
          <div
            ...
            className="group bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer relative overflow-hidden flex flex-col h-full"
          >

{/* ✅ Banner Image */}
 
            
            <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-sky-400 via-primary to-lime-300 opacity-70" />
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-xl p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 transition-all opacity-0 group-hover:opacity-100"
                >
             ...
             {course.bannerUrl && (
    <div className="relative rounded-3xl h-44 w-full overflow-hidden mb-5 border border-slate-200">
      <img
